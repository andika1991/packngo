    ...
    <div id="wrapper">
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            ...
            <div id="content">
             ...
             <!-- Table Pengguna -->
   <!-- Tambah Data Button -->
   <a href="tambahjadwalkapal.php" class="btn btn-primary">Tambah Data</a>
<!-- Modal Tambah Data -->
...
<h3>Jadwal Hari ini</h3>
    ...
    <tbody>
        <?php
        include 'koneksi.php';
        $updateQuery = "UPDATE jadwal_tiket_kapal SET status_jadwal = 'On Going' WHERE waktu_keberangkatan <= NOW()";
        mysqli_query($conn, $updateQuery);
        
        // Update status tiket menjadi "Arrived" jika waktu kedatangan telah tiba
        $updateQuery = "UPDATE jadwal_tiket_kapal SET status_jadwal = 'Arrived' WHERE waktu_kedatangan <= NOW()";
        mysqli_query($conn, $updateQuery);
        $query = "SELECT 
        jtp.id_jadwaltiketkapal,
    ...
    ";
        ...
        if (mysqli_num_rows($result) > 0) {
            while($row = mysqli_fetch_assoc($result)) {
                echo "<tr>";
                echo "<td>" . $row["id_jadwaltiketkapal"] . "</td>";
                ...
                echo "<td>
               <a href='editjadwalkapal.php?id=" . $row["id_jadwaltiketkapal"] . "' class='btn btn-primary btn-sm edit-btn'>Edit</a>
                <button class='btn btn-danger btn-sm delete-btn' data-id_jadwaltiketkapal='" . $row["id_jadwaltiketkapal"] . "' data-toggle='modal' data-target='#deleteModal'>Delete</button>
                      </td>";
                echo "</tr>";
            }
        } else {
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="deleteModalLabel">Hapus Data Jadwal Tiket kapal</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <p>Apakah Anda yakin ingin menghapus data jadwal tiket kapal ini?</p>
            </div>
...
<script>
    $(document).ready(function() {
        $('.delete-btn').click(function() {
            var jadwalId = $(this).data('id_jadwaltiketkapal');
            $('#deleteBtn').attr('href', 'hapustktkpl.php?id_jadwaltiketkapal=' + jadwalId);
        });
    });
</script>
