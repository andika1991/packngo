...
$filename = 'data_vendor_kereta.csv';
...
$query = "SELECT id_vendorkrta, nama_vendor, alamat_vendor, logo_vendor FROM vendor_kereta";
$result = mysqli_query($conn, $query);

// Menuliskan data ke dalam file CSV
while ($row = mysqli_fetch_assoc($result)) {
    $data = array($row['id_vendorkrta'], $row['nama_vendor'], $row['alamat_vendor'], $row['logo_vendor']);
    fputcsv($file, $data);
}
