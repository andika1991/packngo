...
$query = "SELECT id_vendorbus, nama_vendor, alamat_vendor, logo_vendor FROM vendor_bus";
$result = mysqli_query($conn, $query);

// Menuliskan data ke dalam file CSV
while ($row = mysqli_fetch_assoc($result)) {
    $data = array($row['id_vendorbus'], $row['nama_vendor'], $row['alamat_vendor'], $row['logo_vendor']);
    fputcsv($file, $data);
}
